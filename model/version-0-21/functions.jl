...
function display_network(metrics, net, episode) # position in form: [np, app, denp, synp, inp, outp]
    for (i, poss) in enumerate(metrics["net_$(net)_position_episode_$(episode)"])
        l = @layout [a b{0.2w}]
        p1 = Plots.scatter(title="net $(net); episode $episode; iteration $i", leg=false, xlabel="position x", ylabel="position y", xlims=(-init_params["NETWORK_SIZE"], init_params["NETWORK_SIZE"]), ylim=(-init_params["NETWORK_SIZE"], init_params["NETWORK_SIZE"]))
