...
function cross_entropy_learning!(NN, batch, buffer_length; top_percentile=0.5, hidden_range=(0.5,0.99))
    ...
    for b in best
        for (i, (s, a)) in enumerate(shuffle(b))
...
# NETWORK
l1, b1 = create_layer(input_size, 5, buffer_length, weight_init_range, hidden_init_range, init_thresholds=[threshold for _ in 1:5])
l2, b2 = create_layer(5, 5, buffer_length, weight_init_range, hidden_init_range, init_thresholds=[threshold for _ in 1:5])
l3, b3 = create_layer(5, output_size, buffer_length, weight_init_range, hidden_init_range, init_thresholds=[threshold for _ in 1:output_size])
NN = [l1, l2, l3]; BN = [b1, b2, b3]
...
num_iterations_over_min = [0 for _ in 1:training_iterations]; reward_metric = [0. for _ in 1:training_iterations]; for k in 1:training_iterations
    ...
    for i in 1:batch_size
        ...
        for j in 1:simulation_time
            ...
            s = reshape(((Array(s) * Array(s')) .+ 1.) .* 5, length(s)^2)
    ...
    println("avg_reward = $(maximum([b[1] for b in batch]))")
